...
Career-ops는 에이전트 기반으로 작동합니다: Codex, Claude Code, OpenCode가 동일한 버전 관리된 모드, 스크립트, 템플릿, 트래커 흐름으로 라우팅됩니다. 인터랙티브 실행에서는 에이전트가 Playwright로 채용 페이지를 탐색하고, 키워드 매칭이 아닌 이력서와 JD를 비교 분석하여 적합도를 판단하고, 공고별로 이력서를 맞춤 생성할 수 있습니다.
...
| **일괄 처리** | 포함된 배치 러너(Claude 또는 Codex)로 병렬 평가 |
...
> **이 시스템은 사용하는 에이전트가 직접 커스터마이즈하도록 설계되었습니다.** 모드, 아키타입, 스코어링 가중치, 협상 스크립트 -- 그냥 요청하세요. Codex, Claude Code, OpenCode는 자신이 사용하는 동일한 파일을 읽기 때문에 무엇을 수정해야 하는지 정확히 알고 있습니다.
...
- **에이전트**: Codex, Claude Code, OpenCode가 동일한 체크인된 모드와 스크립트를 공유
