...
Cada worker es un proceso hijo del provider seleccionado (`claude` o `codex`) con contexto limpio. El conductor solo orquesta.
...
4. **Para cada URL pendiente**:
   ...
   d. Ejecutar via Bash:
      ```bash
      ./batch/batch-runner.sh --provider {auto|claude|codex}
      ```
...
## Workers (provider-backed)

Cada worker recibe `batch-prompt.md` como contrato de trabajo. Es self-contained.
