...
career-opsはエージェンティックです: Codex、Claude Code、OpenCodeは同じバージョン管理されたモード、スクリプト、テンプレート、トラッカーフローにルーティングされます。インタラクティブ実行では、エージェントがPlaywrightで求人ページを操作し、（キーワードマッチではなく）あなたのCVと求人票を突き合わせて適合度を推論し、求人ごとにレジュメを最適化します。
...
| **バッチ処理** | 同梱のバッチランナー（Claude または Codex）による並列評価 |
...
> **このシステムは使うエージェント自身がカスタマイズする前提で設計されています。** モード、アーキタイプ、スコアリング重み、交渉スクリプト -- Codex、Claude Code、OpenCodeのいずれにも依頼できます。どのエージェントも自分が使うのと同じファイルを読むので、どこを編集すればよいか正確に把握しています。
...
```
career-ops/
├── AGENTS.md                    # Codexのエントリポイント
├── CLAUDE.md                    # 共有プロジェクト指示
├── cv.md                        # あなたのCV（自分で作成）
...
- **エージェント**: Codex、Claude Code、OpenCodeが同じチェックイン済みモードとスクリプトを共有
